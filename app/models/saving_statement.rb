class SavingStatement < ApplicationRecord
  ...
  STATEMENT_TYPES = %i[ cash_deposit cash_withdrawal bank_deposit bank_withdrawal
    ...
    saving_transaction loan_transaction
  ]
  ...
  def set_before_after_balance
    self.before = self.saving_account.balance
    self.after = self.before + (self.is_debit ? self.amount : -self.amount)
  end

  def update_account_balance
    self.saving_account.update!(balance: self.after)
  end
  ...
  def build_associated_transfer
    ...
    daily_interest_saving_account = Account.find_by_code(400)
    transfer.allotments.build({ account: daily_interest_saving_account, is_debit: self.is_debit, amount: self.amount, before: daily_interest_saving_account.balance, after: daily_interest_saving_account.balance + (self.is_debit ? self.amount : -self.amount), transfer: transfer })
  end
